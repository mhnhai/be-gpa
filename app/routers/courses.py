...
def create_course(
    ...
    db_course = crud.create_course(db, course, current_user.id)
    if not db_course:
        raise HTTPException(
...
def get_courses_by_semester(
    ...
    return crud.get_courses_by_semester(db, semester_id, current_user.id)
...
def update_course(
    ...
    course = crud.update_course(db, course_id, current_user.id, course_update)
    if not course:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Không tìm thấy môn học"
...
def delete_course(
    ...
    """Xóa môn học"""
    if not crud.delete_course(db, course_id, current_user.id):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Không tìm thấy môn học"
        )
